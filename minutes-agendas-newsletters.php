...
/*
Copyright 2011  Brenda Egeland (email: brenda@redletterdesign.net)
...
if ( !class_exists( 'MinutesAgendasNewsletters' ) ) {
    ...
    // Constructor
    function MinutesAgendasNewsletters() {
      ...
      <div class=wrap>
        ...
        <div class="explanation">
          ...
          <ul>
            ...
            <li><strong>date format</strong> - if [[[date]]] is used in the link format, it will be formatted according to this format. Valid values
              are the php date formats (see ). The default is 'F j, Y' which formats like January 2, 2011.</li>
            
            <li><strong>file prefix</strong> - when you upload a document, it is renamed to a standard format using the file prefix. Must be alphanumeric (may include -and _ ) and must be unique. The format is fileprefix_YYYY-MM-DD.pdf.</li>
         </ul>
    ...
    function uploadMinutesAgendasNewsletters() {
      ...
      <div class=wrap>
        ...
        if (!$u->documents_exist()) {
          ...
          <p>To delete documents, check the box next to the name and then click the <strong>Delete checked documents</strong> button at the bottom of the table. Careful &mdash; deleted documents can not be recovered.</p>
          <form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
          <?php
    ...
    function minagnews_table_shortcode($atts) {
    ...
    function minagnews_link_shortcode($atts) {
 ...
 * Registers and enqueue the admin CSS
 */
function minagnews_admin_css_custom_page() {
    /** Register */
...
if (!function_exists("minAgNews_adminPanel")) {
  function minAgNews_adminPanel() {
    global $minutesAgendasNewsletters;
    if (!isset($minutesAgendasNewsletters)) {
      return;
    }
    $minagnews_admin_menu = add_options_page('Minutes, Agendas, Newsletters Settings', 'Minutes, Agendas, Newsletters', 'manage_options', basename(__FILE__), array(&$minutesAgendasNewsletters, 'displayAdminSettings'));
    add_action('admin_print_styles-' . $minagnews_admin_menu, 'minagnews_admin_css_custom_page');
  }   
}

// Initialize uploads page
if (!function_exists("minAgNews_uploadPanel")) {
  function minAgNews_uploadPanel() {
    global $minutesAgendasNewsletters;
    if (!isset($minutesAgendasNewsletters)) {
      return;
    }
    $minagnews_admin_menu = add_media_page( 'Manage Minutes, Agendas, Newsletters', 'Minutes, Agendas, Newsletters', 'upload_files', basename(__FILE__), array(&$minutesAgendasNewsletters, 'uploadMinutesAgendasNewsletters'));
    add_action('admin_print_styles-' . $minagnews_admin_menu, 'minagnews_admin_css_custom_page');
...
if (isset($minutesAgendasNewsletters)) {
	...
	// Actions
  // -- display admin settings page
  add_action('admin_menu', 'minAgNews_adminPanel');
  // -- display uploads page
  add_action('admin_menu', 'minAgNews_uploadPanel');
	...
	// Shortcodes
	add_shortcode('minagnews-table', array(&$minutesAgendasNewsletters, 'minagnews_table_shortcode'));
	add_shortcode('minagnews-link', array(&$minutesAgendasNewsletters, 'minagnews_link_shortcode'));
}
